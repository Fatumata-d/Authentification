    ...
    <form method="POST" action="{{ route('login') }}" class="bg-white p-6 rounded-md shadow-md">
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a class="class="underline text-sm text-balck-600 dark:text-black-400 hover:text-black-900 dark:hover:text-dark-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('login') }}">
                {{ __('Déjà inscrit ?') }}">
